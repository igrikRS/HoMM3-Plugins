                     ...
                     )
{
    ...
    case DLL_PROCESS_ATTACH:
        if (!plugin_On)
        {
			...
			_PI->WriteLoHook(0x47625D, TacticSpecHeroCur);	// Атакующий для курсора
			_PI->WriteLoHook(0x475D3A, TacticSpecHeroHex);	// Атакующий для боевой сетки

			_PI->WriteLoHook(0x476268, TacticSpecHeroCur);	// Защитник для курсора
			_PI->WriteLoHook(0x475D5E, TacticSpecHeroHex);	// Защитник для боевой сетки
