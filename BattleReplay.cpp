...
{
	if (isNeedReplay) {
		//b_MsgBox("Обновление карты (пропускаем)", 1);
		c->return_address = 0x4B0AC4; 
		return NO_EXEC_DEFAULT;
	}
	//b_MsgBox("Обновление карты (не пропускаем)", 1);
	return EXEC_DEFAULT; 
}
...
{
	if (isNeedReplay) {
		//b_MsgBox("Добавление опыта (пропускаем)", 1);
		c->return_address = 0x477303;
		return NO_EXEC_DEFAULT;
	}
	//b_MsgBox("Добавление опыта (не пропускаем)", 1);
	return EXEC_DEFAULT; 
}
...
{	
	...
	do {
		...
		if (isNeedReplay) {
			o_QuickBattle = 0;
			CALL_1(void, __cdecl, 0x74CE30, 870520); // аналог !!FU87052:P (важно! можно испортить все переменные)
		}

		ret = CALL_11(int, __thiscall, hook->GetDefaultFunc(), advMng, MixedPos, HrA, MArrA, OwnerD, townD, HrD, MArrD, Pv3, Pv2, Pv1);

		CALL_1(void, __cdecl, 0x74CE30, 870530); // аналог !!FU87053:P (важно! можно испортить все переменные)
	} while ( isNeedReplay );
